...
describe("MiniBar", () => {
  it("renders the title", () => {
    render(<MiniBar contentId="book-1" title="Project Hail Mary" playback={makePlayback()} />);
    expect(screen.getByText("Project Hail Mary")).toBeInTheDocument();
  });

  it("calls togglePlay when the center button is clicked", async () => {
    const togglePlay = vi.fn();
    render(<MiniBar contentId="book-1" title="X" playback={makePlayback({ togglePlay })} />);
  ...
  it("calls onClose when the close button is clicked", async () => {
    const onClose = vi.fn();
    render(<MiniBar contentId="book-1" title="X" playback={makePlayback()} onClose={onClose} />);
  ...
  it("omits the chapter row when no current chapter is known", () => {
    render(
      <MiniBar contentId="book-1" title="X" playback={makePlayback({ currentChapter: null })} />,
    );
