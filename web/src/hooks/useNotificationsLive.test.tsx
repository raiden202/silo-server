...
const frame = (data: object) => ({ type: "event", channel: "notifications", event: "notification.created", data });
...
describe("useNotificationsLive", () => {
  it("toasts and bumps unread count on notification.created", () => {
    const qc = setup(1);
    capturedHandlers.onEvent?.(frame({ id: 5, category: "request", title: "Request approved", body: "Dune", profile_id: "p-1" }));
    expect(toastMock).toHaveBeenCalledWith("Request approved", expect.objectContaining({ description: "Dune" }));
    expect(qc.getQueryData(notificationKeys.unreadCount())).toEqual({ count: 2 });
  });

  it("counts user-wide frames (no profile_id)", () => {
    const qc = setup(0);
    capturedHandlers.onEvent?.(frame({ id: 8, category: "announcement", title: "Maintenance", body: "" }));
    expect(qc.getQueryData(notificationKeys.unreadCount())).toEqual({ count: 1 });
  });

  it("ignores frames for another profile", () => {
    const qc = setup(1);
    capturedHandlers.onEvent?.(frame({ id: 6, category: "content", title: "New content", body: "", profile_id: "p-other" }));
  ...
  it("suppresses restricted categories on child profiles", () => {
    ...
    setup(0);
    capturedHandlers.onEvent?.(frame({ id: 7, category: "request", title: "Request approved", body: "" }));
    expect(toastMock).not.toHaveBeenCalled();
  });

  it("ignores non-created events", () => {
    setup(0);
    capturedHandlers.onEvent?.({ type: "event", channel: "notifications", event: "something.else", data: { id: 1 } });
    expect(toastMock).not.toHaveBeenCalled();
  });
